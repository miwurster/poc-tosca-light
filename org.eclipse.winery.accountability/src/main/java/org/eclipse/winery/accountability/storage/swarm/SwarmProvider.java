...
public class SwarmProvider implements ImmutableStorageProvider {
    ...
    @Override
    public CompletableFuture<String> store(byte[] input) {
        Client client = Client.create();

        return CompletableFuture.supplyAsync(() -> client.resource(swarmUrl)
    ...
    @Override
    public CompletableFuture<byte[]> retrieve(String address) {
        Client client = Client.create();

        return CompletableFuture.supplyAsync(() -> client.resource(swarmUrl + address + "/")
            .type(MediaType.TEXT_PLAIN)
            .accept(MediaType.APPLICATION_OCTET_STREAM)
            .get(byte[].class), getExecutorService());
    }
