...
public class AccountabilityManagerImpl implements AccountabilityManager {
    ...
    @Override
    public CompletableFuture<Map<String, String>> storeState(Map<String, byte[]> files) {
        Objects.requireNonNull(files);
    ...
    @Override
    public CompletableFuture<String> storeState(byte[] data) {
    ...
    @Override
    public CompletableFuture<byte[]> retrieveState(String addressInImmutableStorage) {
    ...
    protected ModelProvenanceElement verifyManifest(List<ModelProvenanceElement> historyElements, String manifestId, Map<String, File> files,
        ...
        if (Objects.nonNull(historyElements) && historyElements.size() > 0) {
            ...
            String checksum = HashingUtil.getChecksum(IOUtils.toByteArray(manifestFile.toURI()), TOSCAMetaFileAttributes.HASH);
            for (ModelProvenanceElement element : historyElements) {
                String retrievedChecksum = HashingUtil.getChecksum(element.getFingerprint().getBytes(), TOSCAMetaFileAttributes.HASH);
    ...
    protected void verifyFiles(ModelProvenanceElement validHistoryElement, Map<String, File> files, Map<String, ProvenanceVerification> verificationMap)
        ...
        for (Map.Entry<String, File> entry : files.entrySet()) {
            ...
            String checksum = HashingUtil.getChecksum(serialize, TOSCAMetaFileAttributes.HASH);
